...
public class EmployeeUserDetailsService implements UserDetailsService {
	...
	public UserDetails loadUserByUsername(String username) throws UsernameNotFoundException {
		...
		//  Can remove below  getRoles is for learning purpose here can use it in SpringSecurityConfig Class with hasRole or Roles Method  
		...
		//        for(EmployeeAuthorities thisAuthority : employeeAuthorities ) {
		//        	authorities = List.of(new SimpleGrantedAuthority(thisAuthority.getAuthority()));
		//        }

		List<SimpleGrantedAuthority> authorities = employeeCredentials.getEmployeeAuthorities().stream().
				map( collectauthorities -> new SimpleGrantedAuthority(
						collectauthorities.getAuthority().toString())).collect(Collectors.toList());
